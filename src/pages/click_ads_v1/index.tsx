    ...
    <div className="">
        ...
        >
          <Row gutter={{ md: 24 }}>
            <Col span={24}>
              <Row gutter={{ md: 24 }}>
                <Col span={12}>
                  <Form.Item label="Từ khóa tìm kiếm:" required>
                    <Form.List
                      name="keywords"
                      initialValue={[""]}
                      rules={[
                          ...
                          validator: async (_, names) => {
                            if (!names || names.length < 1) {
                              return Promise.reject(
                                new Error("Phải có ít nhất 1 từ khóa!")
                              );
                            }
                          },
                        },
                      ...
                      {(fields, { add, remove }) => (
                        <div className="w-[100%]">
                          {fields.map(({ key, name, ...restField }) => (
                            <div
                              key={key}
                              className="flex items-start gap-2.5 justify-between"
                            >
                              <Form.Item
                                style={{ width: "100%" }}
                                className="w-[100%]"
                ...
                <Col span={12}>
                  <Form.Item label="Nhập domain" required>
                    <Form.List
                      name="domain"
                      initialValue={[""]}
                      rules={[
                ...
                <Col span={24}>
                  ...
                  >
                    ...
                    <Button
                      icon={<FaRegStopCircle color="red" />}
                      onClick={onClickStop}
                      className="ml-2.5"
                      color="orange"
                      variant="solid"
      ...
      <div className="flex items-center justify-evenly bg-gray-200 ">
        ...
        <div className="flex items-center bg-red-500 p-2 text-white text-base gap-1 ">
          Số lần click không thành công hoặc không có domain:{" "}
          <strong> {err}</strong>
        </div>
      </div>
          ...
          data.map((item: any, index: number) => {
            return (
              <div key={index} className="px-2">
                {item.start && (
                  <div className="text-green-600">{item.start}</div>
                )}
...
export default ComponentClickAdsV1;
